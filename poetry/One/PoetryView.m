...
-(void)setTextArr:(NSArray *)textArr {
    ...
    NSString *aaa =  [_textArr componentsJoinedByString:@""];
//    NSArray *bbb = [aaa componentsSeparatedByString:@""];
    
    for (int i = 0; i<aaa.length; i++) {
//        NSString *str = [_textArr objectAtIndexVerify:i];
        
        [chinese addObject:[aaa substringWithRange:NSMakeRange(i, 1)]];
    }
...
//    NSArray *chinese = [[_textArr componentsJoinedByString:@""] componentsSeparatedByString:@""];
    ...
    /// 横向
    float Hspace = (self.chineseBgView.width-(14 *itemWithd))/14;
    /// 竖向
...
//    ["十二楼中尽晓妆望仙楼上望君王","曾闻五月到渝州水拍长亭砌下流","人生到处知何似应似非鸿踏雪泥"]
    ...
    for(int i=0; i<chinese.count; i++) {
        ...
        _itemView.frame = CGRectMake((i%14)*itemWithd + Hspace *(i%14)+2, (i/14)*itemHeight + (i/14)*Sspace+10, itemWithd, itemHeight);
        
        _itemView.chineseL.text = [chinese objectAtIndexVerify:i];
//        _itemView.chineseL.font = [UIFont fontWithName:@"zihun125hao-jiuzhouzhenshu" size:50];
